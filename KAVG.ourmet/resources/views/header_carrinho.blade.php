    ...
    <div class="container mx-auto px-6 py-3">
        <div class="flex items-center justify-between">
            <div class="hidden w-full text-gray-600 md:flex md:items-center">
                ...
                <span class="mx-1 text-sm">Governador Valadares</span>
            </div>
            <div class="w-full text-black md:text-center text-2xl font-semibold">
                KAVG.ourmet
            </div>
            <div class="flex items-center justify-end w-full">

                <div class="mr-5 hidden sm:flex">
                ...
                <button @click="cartOpen = !cartOpen" class="text-gray-600 focus:outline-none mx-4 sm:mx-0 " id="carrinho">
...
<div :class="cartOpen ? 'translate-x-0 ease-out' : 'translate-x-full ease-in'" class="fixed right-0 top-0 max-w-xs w-full h-full px-6 py-4 transition duration-300 transform overflow-y-auto bg-white border-l-2 border-gray-300">
    ...
    <div class="flex justify-between mt-6">
        <div class="none" id="produtosContainer">
    ...
    <a class="flex items-center justify-center mt-4 px-3 py-2 bg-red-600 text-white text-sm uppercase font-medium rounded hover:bg-red-500 focus:outline-none focus:bg-blue-500" href="{{ route('cart.removerItem') }}"> 
        <span>Limpar</span>
...
<script>
    ...
    try {
        const response = await fetch('/carrinho/verSessao');
        const data = await response.json();

        const produtosContainer = document.getElementById('produtosContainer');
        produtosContainer.innerHTML = ''; // Limpa o conteúdo do contêiner
        ...
        for (const key in data) {
            if (data.hasOwnProperty(key)) {
                ...
                newItem.classList.add('flex', 'justify-between', 'mt-6');
                newItem.innerHTML = `
                    <div class="flex">
                        <img class="h-20 w-20 object-cover rounded" src="${product.imagem}" alt="${product.name}">
                        <div class="mx-3">
                            <h3 class="text-sm text-gray-600">${product.name}</h3>
                            <div class="flex items-center mt-2">
                                <span class="text-gray-700 mx-2"> Quantidade: ${product.quantidade}</span>
                            </div>
                        </div>
                    </div>
                    <span class="text-gray-600">${product.valor}$</span>
                `;
